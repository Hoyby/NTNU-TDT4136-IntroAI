# TDT4138 - IntroAI Lecture slides
...
If `h_<sub>2</sub>(n)≥ h_<sub>1</sub>(n)` for all `n` (both admissible) then `h_<sub>2</sub>` dominates `h_<sub>1</sub>` and is better for search
...
- For any sentence `\alpha` with variable `x,` `x` is substituted by _new constant symbol_ `k` **that does not appear elsewhere in the knowledge base:**
  ![](img/L9/figure2.png)
  E.g., `∃ y(P(y) ∧ Q(y))` is converted to: `P(CC) ∧ Q(CC)`
...
- If we have a literal `P(x),` where `x` is a variable, in one clause and a complementary literal ` ¬ P(y)` where `y` is a term that does not contain `x,` we can substitute `y` for `x` throughout the first clause.
...
`\theta:\{x_<sub>1</sub>/M; x_<sub>2</sub>/x_<sub>3</sub>; x_<sub>4</sub>/f(..)\}`
...
- Given the KB:
  ` S_<sub>1</sub>: ∀ x, King(x) ∧ Greedy(x)⇒ Evil(x)\\ S_<sub>2</sub>: King(John)\\ S_<sub>3</sub>: ∀ y, Greedy(y)\\`
  We want to infer Evil(John)
- **Unify** `S_<sub>1</sub>` and `S_<sub>2</sub>` by **substituting** `x` with John (shown as {x/John}), and `S_<sub>1</sub>` and `S_<sub>3</sub>` with {y/John}
- Two sentences `\alpha` and `\beta` are unified by `\theta`
  `Unify(\alpha,\beta)=\theta` if `\alpha\theta=\beta\theta`
...
E.g., if we know both `Q` and `Knows(John, x)arrow Likes(John, x)` then we conclude
...
- Consider the two sentences:
  ...
  - `P(y) ∨ R(y)`
- Possible resolvants:
  - `(S(Arvid) ∨ Q(April) ∨ R(Arvid))` `\{y=x, x=Arvid\}`
  - `(S(Sophie) ∨ Q(April) ∨ R(Sophie))` `\{y=x, x=Sophie\}`
  - `(S(x) ∨ Q(April) ∨ R(x))` `\{y=x\}`
...
`S_<sub>1</sub>: ∀ x, King(x) ∧ Greedy(x)⇒ Evil(x)\\ S_<sub>2</sub>: King(John)\\ S_<sub>3</sub>: ∀ y, Greedy(y)\\ S_<sub>4</sub>: Brother(Richard, John)`
...
- `P_<sub>1</sub>'` is `King(John)` and `P_<sub>1</sub>` is `King(x)`
- `\theta` is `\{x/John\}`
- `Q\theta` is `Evil(John)`
...
**Knowledge Base**
`S_<sub>1</sub>:` It is a crime for an American to sell weapons to hostile nations:
`∀ x,y,z, American(x) ∧ Weapon(y) ∧ Sells(x,y,z) ∧ Hostile(z)⇒ Criminal(x)`
`S_<sub>1</sub>: American(x) ∧ Weapon(y) ∧ Sells(x,y,z) ∧ Hostile(z)⇒ Criminal(x)`

`S_<sub>2</sub>:` Nono ... has some missiles
`∃ x, Owns(Nono, x) ∧ Missile(x)`

`S_<sub>2</sub>: Owns(Nono, M_<sub>1</sub>) ∧ Missile(M_<sub>1</sub>)`

`S_<sub>3</sub>:` ... all of its missiles were sold to it by Colonel West
`∀ x, Missile(x) ∧ Owns(Nono, x)⇒ Sells(West, x, Nono)`

`S_<sub>3</sub>: Missile(x) ∧ Owns(Nono, x)⇒ Sells(West, x, Nono)`

`S_<sub>4</sub>:` Missiles are weapons.
`∀ x, Missile(x)⇒ Weapon(x)`

`S_<sub>4</sub>: Missile(x)⇒ Weapon(x)`

`S_<sub>5</sub>:` An enemy of America counts as "hostile"
`∀ x, Enemy(x, America)⇒ Hostile(x)`

`S_<sub>5</sub>: Enemy(x, America)⇒ Hostile(x)`

`S_<sub>6</sub>:` West, who is American...

`S_<sub>6</sub>: American(West)`

`S_<sub>7</sub>:` The country Nono, an enemy of America...

`S_<sub>7</sub>: Enemy(Nono, America)`
...
- Find and use rules that have as premises the known facts in the KB.
  - In Propositional Logic: `A ∧ B⇒ C`
  - In FOL: `King(x) ∧ Greedy(x)⇒ Evil(x)`
...
2. `Owns(Nono, M_<sub>1</sub>)`
3. `Missile(M_<sub>1</sub>)`
...
![](img/L9/figure18.png)
where `UNIFY(l_<sub>j</sub>, ¬ m_<sub>j</sub>)=\theta.`
...
Apply resolution steps to `CNF(KB ∧ ¬\alpha);` complete for FOL.
...
2. Move ` ¬` invards: ` ¬∀ x,p ≡∃ x, ¬ p,\quad ¬∃ x,p ≡∀ x, ¬ p:`
...
- KB:
  ...
  `Owns(Nono, M_<sub>1</sub>) ∧ Missile(M_<sub>1</sub>)`
...
- Facts:
  `American(West), Owns(Nono, M_<sub>1</sub>), Missile(M_<sub>1</sub>), Enemy(Nono, America)`
- Add ` ¬ Criminal(West)`
...
- It is important to note that a percept is about only one time step.
  - For example, proposition **Stench** now may be contradictory if ` ¬`**Stench** is already in the KB - if we don't represent time.
  - There is Stent in time step 3, i.e., **Stench**`^3` will not contradict with ` ¬`**Stench**`^2`
...
- Example **effect axiom:** The agent is at location/square/`[1,1]` and facing east at time `=0` and goes forward. The result is that the agent is in square `[2,1]` and not any longer in `[1,1]` at time `=1`
  `Location_{11}^0 ∧ FacingEast^0 ∧ Forward^0⇒ Location_{21}^1 ∧ ¬ Location_{11}^1`
...
- A solution: The unchanged aspects of world can be represented in form of "Frame axioms" expressing all propositions that remain unchanged. E.g.,
  - `Forward^t⇒(HaveArrow^t\Leftrightarrow HaveArrow^{t+1})`
  - `Forward^t⇒(WumpusAlive^t\Leftrightarrow WumpusAlive^{t+1})`
  - ...
...
- "Successor state axioms"
  `F^{t+1}\Leftrightarrow ActionCauses, F^t ∨(F^t ∧ ¬ ActionCausesNotF^t.)`
- Examples on Successor state axioms:
  - `HaveArrow^{t+1}\Leftrightarrow(HaveArrow^t ∧ ¬ Shoot^t)`
  - `L_{1,1}^{t+1}\Leftrightarrow(L_{1,1}^t ∧( ¬ Forward^t ∨ Bump^{t+1}) ∨(L_{1,2}^t ∧(South^t ∧ Forward^t) ∨(L_{1,3}^t ∧(West^t ∧ Forward^t))`
...
- Another issue: the interference between actions
  ...
  - Example action exclusion axiom: ` ¬ Shoot^t ∨ ¬ Forward^t`
...
- Basic idea:
  ...
  - Uses a restricted form of predicate logic
    ![](img/L10/figure4.png)
    **Action:** `PutOn(r,x,y)`
    **Precondition:** `On(r,x) ∧ Clear(r) ∧ Clear(y)`
    **Effect:** `On(r,y) ∧ Clear(x) ∧ ¬ On(r,x) ∧ ¬ Clear(y)`
...
- The following are not allowed in a state:
  ...
  - ` ¬ BestUniversity(NTNU, Norway)`; not, because of negation.
...
- A goal state is a conjunction of literals (positive/negative) that may contain variables - similar to preconditions.
  E.g., `At(p, Trondheim) ∧ Politican(p)` a/any politician is in Trondheim.
...
2. We could add ` ¬(y=z)` as a precondition for `Move(x,y,z)`
...
In the goal state we have `Have(Milk) ∧ Have(Drill) ∧ Have(Banana)`
...
- Two subgoals, found a plan for each `P_<sub>1</sub>` and `P_<sub>2</sub>`
  - `P_<sub>1</sub>:` MoveToTable(B,A) + Move(A, Table, B)
  - `P_<sub>2</sub>:` MoveToTable(D,C) + Move(C,Table,D)
...
- Move(B, Table, C) must be after MoveToTable(C, A) - otherwise it will do ` ¬`Clear(C) true
...
- A planning graph is a directed graph which is built forward and is organized into levels:
  ...
  - alternating levels `S_<sub>I</sub>` followed by `A_<sub>i</sub>` are built until we reach a termination condition
    - `S_<sub>i</sub>` contains all the literals that could hold at time `i` (even `P` and ` ¬ P`)
    - `A_<sub>i</sub>` contains all the actions that could have their preconditions satisfied at time `i`
...
## Example - graph expanded to level `S_<sub>1</sub>`

![](img/L10/figure31.png)

## Solution extraction (first attempt)

- At level `S_<sub>1</sub>,` all the goal conditions are present, can look for a solution
...
## Example - graph expanded to level `S_<sub>2</sub>`

![](img/L10/figure32.png)

## Solution extraction (second attempt)

- Support ` ¬`garbage with carry, dinner with noop, and present with wrap. This is a _consistent_ set because none are mutually exclusive.
- The subgoals from level `A_<sub>1</sub>` are dinner (precondition for noop), quiet (precondition of wrap)
...
- Start with `S_<sub>n</sub>,` the last level of the planning graphs, and the goals.
- Foe each level `S_<sub>i</sub>` select a number of non-conflicting actions in `A_{i-1}` whose effects cover the goals in `S_<sub>i</sub>.` The resulting state is `S_{i-1}` with goals the precondition of the selected actions.
...
- Level cost is an admissible heuristic but might undercount: it counts the number of levels, wheras there might be several actions at each level `rightarrow` use a serial planning graph
...
2. We can estimate the cost of achieving a goal literal `g_<sub>i</sub>` as the level at which `g_<sub>i</sub>` first appears in the planning graph, the level cost. This estimate is admissible.
...
- All actions an agent `i` can perform are `A_<sub>i</sub>={a_<sub>1</sub>,a_<sub>2</sub>,. . .}`
- All possible outcomes (i.e. outcome space) of the system are `\Omega=\{\omega_<sub>1</sub>,\omega_<sub>2</sub>,. . .\}` where `\omega` represents an outcome corresponding to a collection of actions, one for each agent
...
- So, environmental behaviour is given by the state transformer function:
  `\tau:A_<sub>i</sub> * A_<sub>j</sub>arrow\Omega`
...
- Here is an environment controlled by agent `j` _only:_
  `\tau(D,D)=\omega_<sub>1</sub>\quad\tau(D,C)=\omega_<sub>2</sub>\quad\tau(C,D)=\omega_<sub>1</sub>\quad\tau(C,C)=\omega_<sub>2</sub>`
- Now, a state transformer function of an environemt which is sensitive
  `\tau(D,D)=\omega_<sub>1</sub>\quad\tau(D,C)=\omega_<sub>2</sub>\quad\tau(C,D)=\omega_<sub>3</sub>\quad\tau(C,C)=\omega_<sub>4</sub>`
- And here is another environment where none of the agent has any influence:
  `\tau(D,D)=\omega_<sub>1</sub>\quad\tau(D,C)=\omega_<sub>1</sub>\quad\tau(C,D)=\omega_<sub>1</sub>\quad\tau(C,C)=\omega_<sub>1</sub>`
...
1. Agents `(N>1)`
2. Actions:
   - Each agent `i` chooses an action `a_<sub>i</sub>` from its own action set `A_<sub>i</sub>`
   - The vector `a=(a_<sub>1</sub>,. . .,a_<sub>n</sub>)` of individual actions is alled a **joint action** (or **action profile** or strategy profile). The set `A` is the set of all joint ations.
     ![](img/L11/figure3.png)
3. Utility function:
   - Each agent `i` has its own utility function `u_<sub>i</sub>(a)` that measures the goodness of the joint actions.
...
- Suppose we have the case where both agents can influence the outcome and Action={a,b}, i.e., each agent has just two possible ations that it can perform. Four possible different outcomes can be produced by the system:
  `\tau(a,a)=\omega_<sub>1</sub>\quad\tau(a,b)=\omega_<sub>2</sub>\\ \tau(b,a)=\omega_<sub>3</sub>\quad\tau(b,b)=\omega_<sub>4</sub>`
- Suppose the agents have utility function as follows:
  `u_<sub>i</sub>(\omega_<sub>1</sub>)=1\quad u_<sub>i</sub>(\omega_<sub>2</sub>)=1\quad u_<sub>i</sub>(\omega_<sub>3</sub>)=4\quad u_<sub>i</sub>(\omega_<sub>4</sub>)=4\\ u_<sub>j</sub>(\omega_<sub>1</sub>)=1\quad u_<sub>j</sub>(\omega_<sub>2</sub>)=4\quad u_<sub>j</sub>(\omega_<sub>3</sub>)=1\quad u_<sub>j</sub>(\omega_<sub>4</sub>)=4`

## Utility/Payoff Matrix

- Assume agent `i` has `A_<sub>i</sub>=\{a,b\}` and agent `j` has `A_<sub>j</sub>=\{c,d\}`
- Agent's utilities are shown as a payoff matrix
  ![](img/L11/figure4.png)
- The payoff `u_<sub>1</sub>` is the utility for agent `i` when agent `i` chooses action `a` and agent `j` chooses action `c.`
- The payoff `u_<sub>2</sub>` is the utility for agent `j` when agent `i` chooses action `a` and agent `j` chooses action `c.`
...
- Utility functions lead to preference orderings over outcomes:
  `\omega>_i\omega'` iff `u_<sub>i</sub>(\omega)>u_<sub>i</sub>(\omega')` where `u_<sub>i</sub>` is the utility fn of agent `i`
- Hence, `\omega>_j\omega'` iff `u_<sub>j</sub>(\omega)>u_<sub>j</sub>(\omega')`
- Suppose the agents have utility functions as follows
  `u_<sub>i</sub>(\omega_<sub>1</sub>)=1\quad u_<sub>i</sub>(\omega_<sub>2</sub>)=1\quad u_<sub>i</sub>(\omega_<sub>3</sub>)=4\quad u_<sub>i</sub>(\omega_<sub>4</sub>)=4\\ u_<sub>j</sub>(\omega_<sub>1</sub>)=1\quad u_<sub>j</sub>(\omega_<sub>2</sub>)=4\quad u_<sub>j</sub>(\omega_<sub>3</sub>)=1\quad u_<sub>j</sub>(\omega_<sub>4</sub>)=4`
  where `\omega_<sub>1</sub>=\{D,D\}, \omega_<sub>2</sub>=\{D,C\}, \omega_<sub>3</sub>=\{C,D\}, \omega_<sub>4</sub>=\{C,C\},`
...
- An action is strictly dominant if it strictly dominates every action in `A_<sub>i</sub>.`
...
- Given a game in strategic normal form, an action `a_<sub>i</sub>\in A_<sub>i</sub>` for all player/agent `i` <u>strictly dominates</u> action `b_<sub>i</sub>\in A_<sub>i</sub>` if
  `U_<sub>i</sub>(a_<sub>i</sub>, a_{-i})>U_<sub>i</sub>(b_<sub>i</sub>, a_{-i})\quad∀ a_{-i}\in A_{-i}`
...
- `a_<sub>i</sub>` weakly **dominates** `b_<sub>i</sub>` if
  - `u_<sub>i</sub>(a_<sub>i</sub>, a_{-i})≥ u_<sub>i</sub>(b_<sub>i</sub>, a_{-i})\quad∀ a_{-i}\in A_{-i}` and
  - `u_<sub>i</sub>(a_<sub>i</sub>, a_{-i})>u_<sub>i</sub>(b_<sub>i</sub>, a_{-i})\quad{ for some } a_{-i}\in A_{-i}`
- It is called weakly dominant if it weakly dominates every action in `A_<sub>i</sub>.`
...
- Take a game in strategic form and consider any two actions `a_<sub>i</sub>, b_<sub>i</sub>\in A_<sub>i</sub>` for any player `i\in N.` We say that `a_<sub>i</sub>` is **strictly dominated** by `b_<sub>i</sub>` if
  `u_<sub>i</sub>(a_<sub>i</sub>, a_{-i})<u_<sub>i</sub>(b_<sub>i</sub>, a_{-i})\quad∀ a_{-i}\in A_{-i}`
- We say that `a_<sub>i</sub>` is **weakly dominated** by `b_<sub>i</sub>` if
  - `u_<sub>i</sub>(a_<sub>i</sub>, a_{-i})≤ u_<sub>i</sub>(b_<sub>i</sub>, a_{-i})\quad∀ a_{-i}\in A_{-i}` and
  - `u_<sub>i</sub>(a_<sub>i</sub>, a_{-i})<u_<sub>i</sub>(b_<sub>i</sub>, a_{-i})\quad{ for some } a_{-i}\in A_{-i}`
...
- A solution (i.e., a strategy profile, e.g., `(a_<sub>1</sub>, a_<sub>2</sub>,. . ., a_<sub>n</sub>)`) `S^P` is Pareto optimal, if there is **no** solution `S'` with:
  - `∃` agent `i:U_<sub>i</sub>(S')>U_<sub>i</sub>(S^P)` and
  - `∀` ageng `j:U_<sub>j</sub>(S')≥ U_<sub>j</sub>(S^P)` i.e., none of the others are worse off
...
- The best response of agent `i` is given by `BR_<sub>i</sub>(a_{-i})=\{a_<sub>i</sub>\in A_<sub>i</sub>: u_<sub>i</sub>(a_<sub>i</sub>, a_{-i})≥ u_<sub>i</sub>(b_<sub>i</sub>, a_{-i}{ for all }b_<sub>i</sub>\in A_<sub>i</sub>)\}.`
...
- The strategy profile (defect, defect) is a Nash Equilibrium.
  ...
  - If `j` changes its strategy (defect `arrow` cooperate) it will be worse off (payoff, again, from 2 to 1).
...
- Zero*<sub>s</sub>um encounters are those where utilities sum to zero.
  `u*<sub>i</sub>(\omega)+u\_<sub>j</sub>(\omega)=0\quad∀\omega\in\Omega,{ where }\omega{ is an outcome}`
...
- All members of a category have some properties
  - `(x\in Basketballs)⇒ Spherical(x)`
- All members of a category can be recognized by some properties
  - `Orange(x) ∧ Round(x) ∧ Diameter(x)=9.5" ∧ x\in Balls⇒ x\in Basketballs`
...
- One solution: category `Typical(Tomatoes)`
  - `x\in Typical(Tomatoes)⇒ Red(x) ∧ Spherical(x)`
...
- The `PartOf` predicate is transitive reflexive and transitive
  `PartOf(x,x)\\ PartOf(x,y) ∧ PartOf(y,z)⇒ PartOf(x,z)`
...
- If `KB\models\alpha` then `KB ∧\beta\models\alpha`
...
- A default rule has `3` components:
  ...
  - Justification `(J),`
  - Conclusion `(C)`
- `P:J_<sub>1</sub>. . .,J_<sub>n</sub>\char`\\C`
- If `P` and `J_<sub>1</sub>. . .,J_<sub>n</sub>` cannot be proven false, then the conclusion can be drawn.
...
- Many of the inferences have default status rather than being absolutely certain
  ...
  - Assume knowledge base contains sentence `P` and we want to execute `Tell(KB, ¬ P)`
